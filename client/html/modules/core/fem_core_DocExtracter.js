...
export class DocExtracter {
    dcwDict;
    extract(dcw) {        // returns the dcwFlatTree of a doc [[_s_recId, {S,C}],...] as a DFDict
    ...
    _extract(dcw) { // no async/await as no longer reaching into uncharted territory(into the dch itself)
        ...
        const data = {
            N: FF.getDchName(dcw._s_dch),   // stow name always so we can at least apply dch if importData/exportData fails
            S: style, 
