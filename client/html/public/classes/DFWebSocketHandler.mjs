...
// ----- await  close()                   // close, flush all buffers, cleanup and reset
    ...
    close = async() => {
        if (!this.#closedBy) {
            this.#closedBy = (this.#svr) ? "server" : "client";
        }
        this.#lockId = await this.#lock.lock(this.#lockId);
        if (this.#ws) {
            if (this.#onClose) {
    ...
    _sv_setupWs = (ws, svr) => {
        ...
        this.#ws.onclose = async () => {
            this.#lockId = await this.#lock.lock(this.#lockId);
            this.#closedBy = (this.#svr) ? "client" : "server";
            await this.close();
            this.#lock.unlock(this.#lockId);
...
//     onMessage} );     // f(wsc,msg)  called whenever a message comes in from the client
    ...
    async start(wss, ClientClass) {                     // wss = 'new WebSocketServer({server:yourServerHandle});' or other already opened WebSocketServer
        return new Promise(async (resolve, reject) => {
            wss.on('connection', async (ws) => {
                this.#lockId = await this.#lock.lock(this.#lockId);
                const clientId = this.#nextWSockId++;
                if (!ClientClass) {
                    ClientClass = DFWebSocketClient;
                }
